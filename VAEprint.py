...
class VAEModel():
    ...
    def train(self, train_loader, test_loader, i_max, batch_size, save_path):
        self.model.train()
        ...
        with tqdm(total=total_epochs) as pbar:
            for i in range(i_max + 1):
                ...
                for j in range(3**i):                    
                    ...
                    epoch_counter += 1
                    if(epoch_counter == 1 or epoch_counter == 50 or epoch_counter == 363 ):
                        self.print_images(test_loader, batch_size, self.k, save_path, epoch_counter)
                    pbar.update(1)
    ...
    def print_images(self, test_loader, batch_size, k, savepath, iter):
        self.model.eval()
        ...
        for batch_idx, (x, y) in list(enumerate(test_loader))[:1]:
            ...
            for i in range(columns*rows):
                thetadisp = theta[i].detach().cpu().numpy().reshape(28,28)
                img = np.squeeze(thetadisp)
                # create subplot and append to ax
                ax.append(fig.add_subplot(rows, columns, i+1))
                ax[-1].set_title("label = "+str(y[i]))  # set title
                plt.imshow(img)
            ...
            plt.savefig(savepath+"/"+f"plot{iter}k{k}.jpg")

        self.k = training_k
        self.model.k = training_k
